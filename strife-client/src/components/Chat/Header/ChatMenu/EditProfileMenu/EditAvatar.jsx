...
function EditAvatar(props) {
    ...
    const [showAvatarError, setShowAvatarError] = useState({ show: false, msg: "An error occurred while uploading your avatar." });
    ...
    const handleAvatarFileChanged = async (e) => {
        if (e.target.files[0] != null) {
            ...
            if (fileSize > 0.2) {
                setShowAvatarError({ show: true, msg: "Please select a file that's smaller than 200KB :c" })
            }
            else {
        ...
        <>
            <div hidden={props.value !== props.index} >
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '50px' }}>
                    ...
                    <Typography variant="body1" style={{ color: 'red' }} hidden={!showAvatarError.show}>
                        {showAvatarError.msg}
                    </Typography>
                </div>
            </div>
