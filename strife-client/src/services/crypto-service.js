...
export function encryptSymmetricWithNewKey(inputStr) {
  ...
  const encInputBytesWithNonce = new Uint8Array(
    nonceBytes.length + encInputBytes.length,
  );
  encInputBytesWithNonce.set(nonceBytes);
  encInputBytesWithNonce.set(encInputBytes, nonceBytes.length);
  return {
    secureStoragekeyBase64: encodeBase64(keyBytes),
    encInputWithNonceBase64: encodeBase64(encInputBytesWithNonce),
  };
}
...
export function encryptSymmetric(inputStr, key, isInputInBase64) {
  ...
  const nonceBytes = generateNonce('secretbox');
  const inputStrBytes = isInputInBase64
    ? decodeBase64(inputStr)
    : decodeUTF8(inputStr);
...
export function encryptAsymmetric(inputStr, publicKey, privateKey) {
  ...
  const encInputUint8 = box(
    inputUint8,
    nonceUint8,
    publicKeyUint8,
    privateKeyUint8,
  );
  const encInputWithNonceUint8 = new Uint8Array(
    nonceUint8.length + encInputUint8.length,
  );
...
export function decryptSymmetric(encInputBase64, key, isKeyInBase64) {
  const paddedKeyBytes = isKeyInBase64
    ? decodeBase64(key)
    : decodeUTF8(rightPadKey(key));
  ...
  const encInputBytes = encInputBytesWithNonce.slice(
    secretbox.nonceLength,
    encInputBase64.length,
  );
  const decInputBytes = secretbox.open(
    encInputBytes,
    nonceBytes,
    paddedKeyBytes,
  );
...
export function decryptAsymmetric(encInputBase64, privateKey, publicKey) {
  try {
    ...
    const encInputUint8 = encInputWithNonceUint8.slice(
      box.nonceLength,
      encInputWithNonceUint8.length,
    );
    const decMsgUint8 = box.open(
      encInputUint8,
      nonceUint8,
