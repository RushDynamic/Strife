pipeline {
  ...
  stages {
    stage('Setup Environment Variables') {
      steps {
        script {
          def envStrifeClient = "REACT_APP_AM_API_URL=${REACT_APP_AM_API_URL}\nREACT_APP_SC_API_URL=${REACT_APP_SC_API_URL}\nREACT_APP_CLIENT_URL=${REACT_APP_CLIENT_URL}\nREACT_APP_S3_BUCKET_URL=${REACT_APP_S3_BUCKET_URL}"
          ...
          def envStrifeChatApi = "SC_CORS_ORIGIN_URL_ARRAY=${CORS_ORIGIN_URL_ARRAY}\nAM_API_URL=${AM_API_URL}"

          writeFile(file: 'strife-client/.env', text: envStrifeClient)
